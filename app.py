...
def sign_up(username: str, password: str) -> Optional[str]:
    # Use username as email prefix with a placeholder domain for Supabase
    email = f"{username}@slidesense.local"
    payload = {
        "email": email,
        "password": password,
...
def sign_in(username: str, password: str) -> Optional[str]:
    ...
    # Store username in user metadata for display
    if "user_metadata" not in user:
        user["user_metadata"] = {}
    user["user_metadata"]["username"] = username

    save_user_profile(user, username, access)
    set_session(access, refresh, user)
    return None


def save_user_profile(user: Dict[str, Any], username: str, access_token: str) -> None:
    """Store basic user information in a Supabase table."""
    user_id = user.get("id")
    if not user_id:
        return
    ...
    try:
        ...
        # Ignore profile save errors so login still works
        pass
...
def get_display_name(user: Dict[str, Any]) -> str:
    """Return the username from user metadata, falling back to email prefix."""
